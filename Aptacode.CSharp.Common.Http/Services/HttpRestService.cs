...
namespace Aptacode.CSharp.Common.Http.Services
{
    ...
    public class HttpRestService<TResponse, TRequest>
    {
        ...
        public async Task<HttpServiceResponse<IEnumerable<TResponse>>> GetAll() =>
            await ServiceClient.Send<IEnumerable<TResponse>>(HttpMethod.Get, RouteProvider.Build()).ConfigureAwait(false);

        public async Task<HttpServiceResponse<TResponse>> Get(int id) =>
            await ServiceClient.Send<TResponse>(HttpMethod.Get, RouteProvider.Build(id)).ConfigureAwait(false);

        public async Task<HttpServiceResponse<TResponse>> Update(int id, TRequest content) =>
            await ServiceClient.Send<TResponse, TRequest>(HttpMethod.Post, RouteProvider.Build(id), content).ConfigureAwait(false);

        public async Task<HttpServiceResponse<TResponse>> Create(TRequest content) =>
            await ServiceClient.Send<TResponse, TRequest>(HttpMethod.Put, RouteProvider.Build(), content).ConfigureAwait(false);
